...
const PROVIDER_STYLES = {
  Groq:   { bg: "rgba(99,102,241,0.08)", border: "rgba(99,102,241,0.25)", color: "#818cf8", icon: "⚡" },
  Ollama: { bg: "rgba(63,255,162,0.06)", border: "rgba(63,255,162,0.2)",  color: "#3fffa2", icon: "🦙" },
  none:   { bg: "rgba(239,68,68,0.06)",  border: "rgba(239,68,68,0.2)",   color: "#f87171", icon: "⚠️" },
};
...
export default function Settings({ onToggleSidebar, session, onOpenProfile }) {
  ...
  const style = PROVIDER_STYLES[provider] || PROVIDER_STYLES.none;

  // Section title based on provider
  const engineTitle = isOnline
    ? `AI Engine — ${provider}`
    : "AI Engine";
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl mx-auto flex flex-col gap-5">
          ...
          <Section icon={Cpu} title={engineTitle}>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl transition-all"
              ...
              <div className="flex items-center gap-3">
                {checking ? (
                  ...
                  <Wifi size={16} style={{ color: style.color }} />
                ) : (
                  <WifiOff size={16} className="text-red-400" />
                )}
                <div>
                  <p className="text-sm font-semibold" style={{ color: checking ? "rgba(255,255,255,0.4)" : style.color }}>
                    {checking
                      ? "Checking AI engine…"
                      : isOnline
                        ? `${style.icon} ${provider} is connected`
                  ...
                  {isOnline && aiStatus?.activeModel && (
                    <p className="text-xs text-white/35 mt-0.5">
                      Active model:{" "}
                      <span className="font-mono" style={{ color: style.color + "aa" }}>
            ...
            {isOnline && provider === "Groq" && aiStatus?.availableModels?.length > 0 && (
              <Field label="Available Models" hint="These models are available via your Groq API key.">
                <div className="flex flex-col gap-2">
                  {aiStatus.availableModels.map(m => (
                    <div key={m}
                      className="flex items-center justify-between px-3.5 py-2.5 rounded-xl transition-all"
                      style={{
                        background: aiStatus.activeModel === m ? "rgba(99,102,241,0.1)" : "rgba(255,255,255,0.03)",
                        border: `1px solid ${aiStatus.activeModel === m ? "rgba(99,102,241,0.3)" : "rgba(255,255,255,0.06)"}`,
                      }}>
                      <div className="flex items-center gap-2.5">
                        <span className="text-base">⚡</span>
                        <span className="text-sm font-mono text-white/75">{m}</span>
                      </div>
                      {aiStatus.activeModel === m && (
            ...
            {isOnline && provider === "Ollama" && aiStatus?.availableModels?.length > 0 && (
              <Field label="Installed Models" hint="These models are installed locally via Ollama.">
                <div className="flex flex-col gap-2">
                  {aiStatus.availableModels.map(m => (
                    <div key={m}
                      className="flex items-center justify-between px-3.5 py-2.5 rounded-xl transition-all"
                      style={{
                        background: aiStatus.activeModel === m ? "rgba(63,255,162,0.08)" : "rgba(255,255,255,0.03)",
                        border: `1px solid ${aiStatus.activeModel === m ? "rgba(63,255,162,0.25)" : "rgba(255,255,255,0.06)"}`,
                      }}>
                      <div className="flex items-center gap-2.5">
                        <span className="text-base">🦙</span>
                        <span className="text-sm font-mono text-white/75">{m}</span>
              ...
              <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,107,53,0.15)" }}>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="p-3 rounded-xl" style={{ background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.15)" }}>
                    ...
                    <p className="text-xs text-white/40 mb-2">Get a free API key at console.groq.com then add to your server:</p>
                    ...
                    <div className="space-y-1.5">
                      {[
                        ...
                        <div key={n} className="flex items-start gap-2">
                          <span className="w-4 h-4 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0 mt-0.5"
